...
mkdir -p log
for ((thread_count = 1; thread_count <= 32; thread_count*=2)); do
...
for size in ${sizes[*]}; do
    ...
    for ((i = 0; i < $test_count; i++)); do
        make gen size=$size > /dev/null
        output="$(make serial)"
        echo "$output" | grep "time" >> "./log/serial/${size}_${thread_count}"
        if [[ $i == 0 ]]; then
            echo "$output" | grep -v "time" >> "./log/serial/${size}_${thread_count}_out"
        fi
