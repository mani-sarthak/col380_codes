...
vector<data_type> convolve3d1(vector<data_type>&input, vector<data_type>&kernel, int inputsize, int kernelsize, int depth){
...
vector<data_type> convolve3d2(vector<data_type> &input, vector<data_type>&kernel, int inputsize, int kernelsize, int depth,int channels){
...
void read_weight_file(string filename, vector<data_type> &kernel, vector<data_type> &bias, int dim1,int dim2, int depth) {
...
void lenet(string filename){
    ...
    vector<float> out1 = convolve3d1(img,conv1_weights,28,5,20);
    addbias(out1,conv1_bias,24*24,20);
    vector<float> out2 =  pool3d(out1,24,2,20);
    vector<float> out3 = convolve3d2(out2,conv2_weights,12,5,20,50);
    addbias(out3,conv2_bias,8*8,50);
    vector<float> out4 = pool3d(out3,8,2,50);
    vector<float> out5 = convolve3d2(out4,fc1_weights,4,4,50,500);
    addbias(out5,fc1_bias,1*1,500);
    vector<float> out6 = relu(out5);
    vector<float> out7 = convolve3d2(out6,fc2_weights,1,1,500,10);
